...
import { Gizmo, UtilTmp } from "./util";
...
export class GMath {
    ...
    /** 取夹值,会改变value（在[min,max]区间取value,在区间外取min,max） */
    ...
    public static StretchedBillboardAngle(upward: Vec3, current: Vec3, target: Vec3, nodeForward: Vec3): number | undefined {
        ...
        if (target == null) {
            console.warn('StretchedBillboardAngle is null.');
            return;
        }

        UtilTmp.V3_0.set(target);
        UtilTmp.V3_1.set(nodeForward);
        UtilTmp.V3_2.set(target);

        const targetForward = UtilTmp.V3_0.subtract(current);
        const costValue = Vec3.dot(targetForward, nodeForward);
        const projectPoint = UtilTmp.V3_1.multiplyScalar(costValue);
        const projectForward = UtilTmp.V3_2.subtract(projectPoint);
